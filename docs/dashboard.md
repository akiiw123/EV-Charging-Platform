...
- 站内电子轨道（fx 层绘制）：电桩环绕站心排布在虚线轨道环上；轨道半径自适应 = 最近邻站距 × 45%（0.025~0.12°），间距小于 0.06° 的近邻站按簇错位（质心圆周均匀摆开，仅影响绘制、文档标注示意），保证每站的电桩环绕自己的电站、环线相切不穿插；轨道虚线缓慢旋转，沿环公转的"电子"能量点数量 = 充电中桩数 + 1，环上随机闪过火花弧段（夜间亮金、日间橙红）模拟电流滋滋声；夜间另有桩点星辉光晕。
