# QML 运营管理端

管理端采用 Qt Quick/QML 实现桌面应用，主要适配 1440×900，最低窗口尺寸为 1280×720。数据库、TCP/JSON 协议、密码校验及用户端业务逻辑保持兼容。

## 设计系统

共享模块位于 `libs/ui`，模块 URI 为 `Charging.UI`。管理端统一使用语义令牌，不在页面中根据主题逐项判断颜色。

- 默认品牌色：`#0F9F8F`，与用户端青绿品牌保持一致。
- 默认背景/表面：`#F4F7FA` / `#FFFFFF`。
- 深海主题背景/表面/强调色：`#07111F` / `#102039` / `#18C8F4`。
- 字号：11 / 13 / 15 / 20 / 28 px，并支持 85%～130% 缩放。
- 间距：4 px 起步的 8 px 主栅格；页面边距 24 px，模块间距 12～18 px。
- 圆角：8 / 12 / 16 px；卡片为 1 px 语义边框。
- 动画：140 ms 控件反馈、200 ms 页面与抽屉过渡，使用 OutCubic，无弹跳和持续闪烁。
- 图标：统一使用 `LineIcon` Canvas 线性图标，不混用 Emoji 或位图。
- 状态：成功、品牌、信息、警告、危险和离线均同时显示文字，不仅依赖颜色。

主题共七套：默认青绿、深海青蓝、极光紫、石墨橙、翡翠绿、云白蓝、高对比。主题、侧栏状态、动画、字体缩放和表格页大小通过 `QSettings` 持久化。

账号安全：侧边栏底部提供“安全设置”入口，管理员可在任意时刻打开修改密码弹窗；首次登录检测到初始密码时仍会强制提醒（每次登录最多提醒一次，可稍后处理）。修改成功后服务端要求使用新密码重新登录。

## 代码结构

- `libs/ui/qml/Theme.qml`：共享语义令牌与七套主题。
- `libs/ui/qml/`：共享卡片、按钮、输入框、状态徽标和线性图标。
- `apps/admin-server/qml/components/`：桌面壳层、侧栏、顶栏、数据表、弹窗和详情抽屉。
- `apps/admin-server/qml/pages/`：登录、总览、电站、电桩、订单、用户、预测和设置。
- `apps/admin-server/src/admin_app_controller.*`：页面状态、异步 TCP/HTTP 请求、筛选和设置。
- `apps/admin-server/src/json_list_model.h`：面向 QML 代理复用的数据列表模型。
- `libs/core/src/request_router.cpp`：已有管理协议及新增订单列表、电站编辑/删除接口。

页面层不执行 SQL。数据库操作仍由 TCP 服务端路由完成；机器学习推理由独立 Python HTTP 服务执行，不阻塞 GUI 线程。

## 电站与电桩管理联动

- **电站三级区域筛选**：电站管理页筛选栏提供 省→市→区 三个联动下拉。选项由当前电站数据的去重值实时生成（不内置行政区划字典，保证没有“选了上级、下级无选项”的死选项）；上级变化会重置下级，区域为空的电站归入“未分区”。区域字段自数据库版本 6 引入，可在“新增/编辑电站”表单中维护；筛选在客户端按字段精确匹配，与名称/地址搜索叠加。
- **去管理电桩**：电站详情抽屉提供“去管理电桩（N 台）”按钮，点击后切换到电桩管理页并自动带上该电站的预筛选；筛选以已选标签的形式显示在筛选栏下方，可逐个移除或一键清除，不会锁死上下文。该跳转会把电站记入“最近管理”（见下）。
- **电桩管理所属电站多选**：电桩管理页筛选栏最左为“所属电站”多选下拉（`MultiSelectComboBox`），支持站内搜索、勾选多个电站、超长列表折叠与“展开全部”；面板顶部展示“最近管理”推荐区。不选择任何电站时展示全部电桩；与编号搜索、类型、状态筛选叠加生效。筛选按电站 `station_id` 精确匹配（不依赖电站名称，重名/改名安全）。
- **最近管理（常用电站推荐）**：控制器通过 `QSettings`（键 `recent/stationIds`）本机记录最近被管理的最多 5 个电站（跳转预筛选或在电桩页单选筛选都会记入）。数据真实可解释；限制：按机器记录，不区分管理员账号，不跨设备同步。
- **计价规则编辑**：电站详情抽屉提供“计价规则”按钮，打开 `PricingDialog` 可维护该站的分时电价段（当日分钟区间，左闭右开，类型 峰/平/谷）与占位费参数（免费挪车分钟数、每分钟费率、封顶，占位费不受启用开关影响）。打开时通过 `admin.pricing.get` 拉取，保存走 `admin.pricing.set`，时段越界/重叠等校验由服务端完成并以提示条反馈；保存后立即对后续充电计费生效（分段积分 + 占位费并入待结算金额）。未启用分时电价或时段未覆盖的时刻回退到电站固定单价。

## 构建

```bash
cd /home/bit/EV-Charging-Platform
cmake -S . -B build/admin-qml2 -DCMAKE_BUILD_TYPE=Debug
cmake --build build/admin-qml2 -j2
```

完整 Ubuntu 依赖（包括用户端地图所需的 Qt WebEngine 辅助进程）见仓库根目录 `README.md`。

## 启动

在 VMware 的 Ubuntu 图形桌面终端运行：

```bash
cd /home/bit/EV-Charging-Platform
bash scripts/run-desktop.sh admin
```

管理端进程会同时初始化 SQLite 数据库并启动 TCP 服务。随后可在另一个终端启动用户端：

```bash
bash scripts/run-desktop.sh user
```

VS Code Remote SSH 终端通常没有图形显示连接。如果 `/tmp/.X11-unix/X0` 存在，可先设置 `DISPLAY=:0`；否则应在 VMware 桌面终端启动，不要使用 `offscreen` 做人工演示。

## 智能预测

预测服务由队友的 `ml/service.py` 提供：

```bash
cd /home/bit/EV-Charging-Platform/ml
python3 service.py --data-dir ./data --artifacts ./artifacts --port 8090
```

管理端默认访问 `http://127.0.0.1:8090`，可通过 `CHARGING_ML_URL` 覆盖。`sessions` 后端会明确标记为“历史回放模型”并显示回放时点；未来 1/6/24 小时指标按曲线累加，不能把第 6/24 个小时误写成累计值。点预测未提供分位数区间时显示“未提供”，不会显示虚假的 0% 可信度。服务或模型未准备好时，预测页降级为最近 24 小时平台真实订单估算，不使用固定演示值。

预测页先读取 `/health`，再读取 `/stations` 和 `/predict`。页面直接展示接口返回的模型版本、`selected_models.energy_kwh`、可用站点总数、数据时间范围、年份平移标识、容量核验状态、负荷构造口径和 warnings。管理端只并发推理前 6 个站点，汇总卡和状态栏必须明确写成“当前展示/汇总 6 个站点”，不能冒充 76 个站点的全量结果。

## 验证

```bash
ctest --test-dir build/admin-qml2 --output-on-failure
```

截至 2026-09-08，全量构建通过；7 个 CTest 程序中 6 个通过。`tcp-integration-tests` 的 `reconnectAfterInitialRefusal` 用例仍失败，表示首次连接被拒后的自动重连尚需修复；其余 TCP 集成用例通过。

无图形环境可验证 QML 加载：

```bash
timeout 12s env QT_QPA_PLATFORM=offscreen QT_QUICK_BACKEND=software \
  ./build/admin-qml2/apps/admin-server/charging-admin
```

退出码 124 且没有 QML 日志表示窗口持续运行并成功加载。

## 当前限制

- 数据库尚无“电桩最近心跳”字段，表格明确显示“暂无心跳字段”；未伪造心跳时间。
- 当前电站删除受历史订单外键保护；包含关联订单（含已完成/已取消的历史订单）的电站会被服务端拒绝删除，并返回明确提示“电站存在历史订单记录，为保留订单数据不能删除”。
- 机器学习服务需要训练产物和数据目录。服务不可用时自动降级为按最近 24 小时真实订单与实时空闲桩做历史数据估算，页面以“历史数据估算”徽标明确标识，并提示启动 `ml/service.py` 后自动切换为模型预测。
- 表格已使用 C++ 模型、搜索、状态筛选和滚动；分页设置已持久化，但服务端分页协议尚未实现，当前接口最多返回 200～500 条。
- 电站详情展示已有汇总字段；数据库暂无电站独立“更新时间”列，继续显示创建时间，未伪造更新时间。
- 区域筛选选项来自电站数据的去重值，按 Unicode 排序（非拼音序）；旧库升级后区域字段为空，需在“编辑电站”中补填，回填保护保证种子数据不会覆盖管理员修改。
- “最近管理”为本机 `QSettings` 记录（不区分管理员账号、不跨设备同步）；区域筛选、电站多选均在客户端过滤，服务端分页协议尚未实现（数据量小时客户端过滤足够）。
