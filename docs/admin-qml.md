...
管理端采用 Qt Quick/QML 实现桌面应用，主要适配 1440×900，最低窗口尺寸为 1280×720。数据库、TCP/JSON 协议、密码校验及用户端业务逻辑保持兼容。
...
账号安全：侧边栏底部提供“安全设置”入口，管理员可在任意时刻打开修改密码弹窗；首次登录检测到初始密码时仍会强制提醒（每次登录最多提醒一次，可稍后处理）。修改成功后服务端要求使用新密码重新登录。
...
- 当前电站删除受历史订单外键保护；包含关联订单（含已完成/已取消的历史订单）的电站会被服务端拒绝删除，并返回明确提示“电站存在历史订单记录，为保留订单数据不能删除”。
- 机器学习服务需要训练产物和数据目录。服务不可用时自动降级为按最近 24 小时真实订单与实时空闲桩做历史数据估算，页面以“历史数据估算”徽标明确标识，并提示启动 `ml/service.py` 后自动切换为模型预测。
