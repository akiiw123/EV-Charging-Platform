/*
 * 文件职责：电桩管理页：实现电站筛选、电桩编辑、状态修改与重启确认，对接 AdminAppController。
...
Item {
    ...
    // 所属电站多选状态(电站 id 数组;空数组=全部电桩)
    property var selectedStationIds: []
    property var stationSummaries: []   // 多选下拉全量选项
    property var recentStations: []     // "最近管理"推荐(控制器 QSettings 记录)
    readonly property var selectedChips: {
        var out = []
        for (var i = 0; i < selectedStationIds.length; i++) {
            var station = findStation(selectedStationIds[i])
            if (station) out.push(station)
        }
        return out
    }

    Component.onCompleted: { reloadStationOptions(); runQuery() }
    Connections {
        target: adminController
        function onStationDataChanged() { reloadStationOptions() }
    }

    function findStation(id) {
        for (var i = 0; i < stationSummaries.length; i++)
            if (Number(stationSummaries[i].id) === Number(id)) return stationSummaries[i]
        return null
    }
    function reloadStationOptions() {
        root.stationSummaries = adminController.allStationSummaries()
        root.recentStations = adminController.recentStations()
    }
    function runQuery() {
        adminController.refreshPiles(q.text, root.selectedStationIds,
            type.currentIndex === 1 ? "fast" : type.currentIndex === 2 ? "slow" : "",
            ["", "idle", "charging", "fault", "offline"][state.currentIndex])
    }
    function setSelectedStationIds(ids) {
        root.selectedStationIds = ids
        runQuery()
    }
    // 电站管理页"去管理电桩"跳转入口:预筛选该站(可通过标签清除恢复全部)
    function applyStationFocus(station) {
        setSelectedStationIds([Number(station.id)])
        adminController.notify("已在电桩管理中筛选「" + (station.name || "") + "」", "success")
    }
    ...
    ColumnLayout { anchors.fill: parent; anchors.margins: 24; spacing: 16
        ...
        RowLayout { Layout.fillWidth: true; spacing: 8
            MultiSelectComboBox {
                ...
                placeholder: "所属电站：全部"
                options: root.stationSummaries
                recentOptions: root.recentStations
                selectedIds: root.selectedStationIds
                onSelectionChanged: function(ids) { root.setSelectedStationIds(ids) }
            }
            SearchField { id: q; Layout.fillWidth: true; placeholderText: "搜索电桩编号" }
            FilterComboBox { id: type; Layout.preferredWidth: 108; model: ["全部类型", "快充", "慢充"] }
            FilterComboBox { id: state; Layout.preferredWidth: 108; model: ["全部状态", "空闲", "充电中", "故障", "离线"] }
            AppButton { text: "查询"; onClicked: root.runQuery() }
            Item { Layout.fillWidth: true }
            AppButton { text: "刷新"; variant: "secondary"; onClicked: root.runQuery() }
            AppButton { text: "新增电桩"; onClicked: { root.selected = {}; editor.open() } }
        }
        // 已选电站标签:跳转预筛选后在此可见,可逐个移除或一键清除(空选择=全部电桩)
        RowLayout {
            Layout.fillWidth: true; spacing: 8
            visible: root.selectedChips.length > 0
            Text {
                text: root.selectedChips.length > 3 ? "已选 " + root.selectedChips.length + " 个电站" : "已选电站"
                color: Theme.textMuted; font.pixelSize: Theme.fontCaption
            }
            Repeater {
                model: root.selectedChips.length > 3 ? [] : root.selectedChips
                delegate: Rectangle {
                    required property var modelData
                    Layout.preferredHeight: 26
                    Layout.preferredWidth: chipText.implicitWidth + 36
                    ...
                    Text {
                        anchors.right: chipClose.left; anchors.rightMargin: 2; anchors.verticalCenter: parent.verticalCenter
                        text: "×"; color: Theme.textMuted; font.pixelSize: Theme.fontCaption
                    }
                    MouseArea {
                        id: chipClose
                        ...
                        onClicked: {
                            var next = []
                            for (var i = 0; i < root.selectedStationIds.length; i++)
                                if (Number(root.selectedStationIds[i]) !== Number(modelData.id)) next.push(root.selectedStationIds[i])
                            root.setSelectedStationIds(next)
                        }
                    }
                }
            }
            Item { Layout.fillWidth: true }
            AppButton { text: "清除筛选"; variant: "secondary"; onClicked: root.setSelectedStationIds([]) }
        }
